...
// Làm việc với danh mục 
  ...
  getDanhMucById(category_id: any): Observable<any> {  
    return this.http.get(`${this.apiUrl}/Categories/${category_id}`);  
  }

  suaDanhMuc(CategoryId: number, categoryData: any): Observable<any> {
    return this.http.put<any>(`${this.apiUrl}/Categories/UpdateCategories/${CategoryId}`,categoryData);
  }
  themDanhMuc(val:any){
